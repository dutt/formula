...
class Assets:
    ...
    def setup_actual(self):
        ...
        self.reptile_sheet = Spritesheet(
            util.resource_path("data/graphics/DawnLike/Characters/Reptile")
        )
        self.undead_sheet = Spritesheet(
            util.resource_path("data/graphics/DawnLike/Characters/Undead")
        )
        self.humanoid_sheet = Spritesheet(
            util.resource_path("data/graphics/DawnLike/Characters/Humanoid")
        )
        self.quadraped_sheet = Spritesheet(
            util.resource_path("data/graphics/DawnLike/Characters/Quadraped")
        )
        self.player_sheet = Spritesheet(
            util.resource_path("data/graphics/DawnLike/Characters/Player")
        )
        self.dog_sheet = Spritesheet(
            util.resource_path("data/graphics/DawnLike/Characters/Dog")
        )

        self.wall_sheet = Spritesheet(
            util.resource_path("data/graphics/DawnLike/Objects/Wall.png")
        )
        self.decor_sheet = Spritesheet(
            util.resource_path("data/graphics/DawnLike/Objects/Decor")
        )
        self.floor_sheet = Spritesheet(
            util.resource_path("data/graphics/DawnLike/Objects/Floor.png")
        )
        self.effect_sheet = Spritesheet(
            util.resource_path("data/graphics/DawnLike/Objects/Effect")
        )
        self.trap_sheet = Spritesheet(
            util.resource_path("data/graphics/DawnLike/Objects/Trap")
        )

        self.potion_sheet = Spritesheet(
            util.resource_path("data/graphics/DawnLike/Items/Potion.png")
        )
        self.medium_weapons_sheet = Spritesheet(
            util.resource_path("data/graphics/DawnLike/Items/MedWep.png")
        )
        self.ammo_sheet = Spritesheet(
            util.resource_path("data/graphics/DawnLike/Items/Ammo.png")
        )
        self.money_sheet = Spritesheet(
            util.resource_path("data/graphics/DawnLike/Items/Money.png")
        )
        self.key_sheet = Spritesheet(
            util.resource_path("data/graphics/DawnLike/Items/Key.png")
        )
        self.scroll_sheet = Spritesheet(
            util.resource_path("data/graphics/DawnLike/Items/Scroll.png")
        )

        if (
            graphics_file_tile_size.width != CELL_WIDTH
            or graphics_file_tile_size.height != CELL_HEIGHT
        ):
        ...
        def get_wall(sheet, col, row):  # col, row are coords of the top left sprite
            ...
            retr.append(
                get_img(sheet, col + 1, row + 1, rotate=0)
            )  # 1, wall to the north
            retr.append(
                get_img(sheet, col + 1, row + 0, rotate=0)
            )  # 2, wall to the east
            retr.append(
                get_img(sheet, col + 0, row + 2, rotate=0)
            )  # 3, wall to the north, east
            retr.append(
                get_img(sheet, col + 1, row + 1, rotate=180)
            )  # 4, wall to the south
            retr.append(
                get_img(sheet, col + 0, row + 1, rotate=0)
            )  # 5, wall to the north, south
            retr.append(
                get_img(sheet, col + 0, row + 0, rotate=0)
            )  # 6, wall to the east, south
            retr.append(
                get_img(sheet, col + 3, row + 1, rotate=0)
            )  # 7, wall to the north, south, east
            retr.append(
                get_img(sheet, col + 1, row + 0, rotate=0)
            )  # 8, wall to the west
            retr.append(
                get_img(sheet, col + 2, row + 2, rotate=0)
            )  # 9, wall to the north, west
            retr.append(
                get_img(sheet, col + 1, row + 0, rotate=0)
            )  # 10, wall to the east, west
            retr.append(
                get_img(sheet, col + 4, row + 2, rotate=0)
            )  # 11, wall to the north, east, west
            retr.append(
                get_img(sheet, col + 2, row + 0, rotate=0)
            )  # 12, wall to the south, west
            retr.append(
                get_img(sheet, col + 5, row + 1, rotate=0)
            )  # 13, wall to the north, south, west
            retr.append(
                get_img(sheet, col + 4, row + 0, rotate=0)
            )  # 14. wall to the east, south, west
            retr.append(
                get_img(sheet, col + 4, row + 1, rotate=0)
            )  # 15, wall to everywhere
            return retr
        ...
        def get_floor(sheet, col, row):  # col, row are coords for the top left sprite
            retr = [
                ...
                get_img(
                    sheet, col + 0, row + 2, rotate=0
                ),  # 3, floor to the north, east
                get_img(sheet, col + 1, row + 1, rotate=180),  # 4, floor to the south
                get_img(
                    sheet, col + 3, row + 1, rotate=0
                ),  # 5, floor to the north, south
                get_img(
                    sheet, col + 0, row + 0, rotate=0
                ),  # 6, floor to the east, south
                get_img(
                    sheet, col + 0, row + 1, rotate=0
                ),  # 7, floor to the north, south, east
                get_img(sheet, col + 6, row + 1, rotate=0),  # 8, floor to the west
                get_img(
                    sheet, col + 2, row + 2, rotate=0
                ),  # 9, floor to the north, west
                get_img(
                    sheet, col + 5, row + 1, rotate=0
                ),  # 10, floor to the east, west
                get_img(
                    sheet, col + 1, row + 2, rotate=0
                ),  # 11, floor to the north, east, west
                get_img(
                    sheet, col + 2, row + 0, rotate=0
                ),  # 12, floor to the south, west
                get_img(
                    sheet, col + 2, row + 1, rotate=0
                ),  # 13, floor to the north, south, west
                get_img(
                    sheet, col + 1, row + 0, rotate=0
                ),  # 14. floor to the east, south, west
        ...
        self.font_title = pygame.font.Font(
            util.resource_path("data/font/CutiveMono-Regular.ttf"), 20
        )
        ...
        self.font_message = pygame.font.Font(
            util.resource_path("data/font/rm_typerighter_old.ttf"), 38
        )
        ...
        self.font_console = pygame.font.Font(
            util.resource_path("data/font/Consolas.ttf"), 20
        )
