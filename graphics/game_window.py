    ...
    def draw(self, game_data, gfx_data):
            ...
            for x in range(gfx_data.camera.x1, gfx_data.camera.x2):
                for y in range(gfx_data.camera.y1, gfx_data.camera.y2):
                    ...
                    if asset:
                        ...
                        drawable.colorize(
                            (darken, darken, darken), pygame.BLEND_RGBA_SUB
                        )
            ...
            for e in rendering_sorted:
                ...
                if tcod.map_is_in_fov(
                    ...
                    for ad in e.attached_effects:
                        main.blit(
                            ad.drawable.asset, (sx * CELL_WIDTH, sy * CELL_HEIGHT)
                        )
                    ...
                    if e.fighter and e.ai and e.fighter.hp != e.fighter.max_hp:
                        bar_pos = Pos(
                            sx * CELL_WIDTH,
                            (CELL_HEIGHT - bar_height) + sy * CELL_HEIGHT,
                        )
                        display_bar(
                            main,
                            assets=None,
        ...
        def draw_targeting():
            ...
            if dist > max_dist:
                ...
                pygame.draw.line(
                    targeting_surface, (100, 100, 100), red_part, rect_center
                )
                draw_rect_boundary(
                ...
                draw_rect_boundary(
                    targeting_surface, (255, 0, 0), rect, draw_cross=False
                )
            else:
        ...
        def draw_mouse_over_info():
            ...
            px, py = pos
            map_screen_pos = global_screen_pos_to_map_screen_pos(px, py)
            tile_x, tile_y = map_screen_pos_to_tile(
                map_screen_pos[0], map_screen_pos[1]
            )
