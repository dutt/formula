...
class Tutorial:
    ...
    def get_messages(game_data, gfx_data):
        ...
        if not game_data.stats.moves:
            ...
            messages.append(
                Message(
                    "You can also left-click on explored tiles to move there", Pos(x, y + linediff), MessageType.LINE,
                )
            )
            ...
            if config.conf.consumables:
                ...
                messages.append(
                    Message(
                        "Down to the left are consumables, single use items that can help", Pos(0, y), MessageType.LINE,
                    )
                )
            ...
            messages.append(Message("Kill it the second Formula, FRR", Pos(x, y + linediff), MessageType.LINE,))
            messages.append(
                Message("Press 2, or click the formula, to target", Pos(x, y + linediff * 2), MessageType.LINE,)
            )

            messages.append(
                Message("Defensive formulas will always target yourself", Pos(x, y + linediff * 3), MessageType.LINE,)
            )
            ...
            messages.append(
                Message("Move onto it and press space to pick it up", Pos(x, y + linediff), MessageType.LINE,)
            )
            messages.append(
                Message("When you do, you'll get a little loot", Pos(x, y + linediff * 2), MessageType.LINE,)
            )
            ...
            for e in game_data.map.entities:
                if e.stairs and tcod.map_is_in_fov(game_data.fov_map, e.pos.x, e.pos.y):
                    ...
                    sy += 75
                    messages.append(
                        Message("1) You will get to change your formulas", Pos(sx + 30, sy), MessageType.LINE,)
                    )
                    messages.append(
                        Message("2) Your cooldowns will reset", Pos(sx + 30, sy + linediff), MessageType.LINE,)
                    )
                    messages.append(
                        Message("3) Your shield will remain", Pos(sx + 30, sy + linediff * 2), MessageType.LINE,)
                    )
                    break
            ...
            if has_killed and (has_not_looted or has_cooldown):
                ...
                messages.append(
                    Message("Cooldown reduce when you explore new tiles", Pos(20, 220 + linediff), MessageType.LINE,)
                )
