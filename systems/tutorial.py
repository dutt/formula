...
class Tutorial:
    ...
    def get_messages(game_data, gfx_data):
        ...
        if game_data.player.pos == first_pos:
            ...
            messages.append(Message("Move right for tutorial, move down to skip", Pos(welcome_py+40, welcome_py+40), MessageType.LINE))
            ...
            messages.append(Message("This will be is your shield bar", Pos(20, 80), MessageType.LINE))
            ...
            messages.append(Message("FFR is Fire, Fire, Range. Short range, high damage", Pos(20, 220), MessageType.LINE))
            messages.append(Message("FRR is Fire, Range, Range. Longer range, low damage", Pos(20, 260), MessageType.LINE))

        elif game_data.player.pos == fourth_pos:
            messages.append(
                Message("This first enemy is inactive, others won't be.", Pos(80, 160), MessageType.LINE))
            messages.append(Message("Move closer, then kill it the first Formula, FFR", Pos(80, 180), MessageType.LINE))
            messages.append(Message("Press 1, or click the formula, to target", Pos(80, 200), MessageType.LINE))
            messages.append(Message("Defensive formulas will always target yourself", Pos(80, 240), MessageType.LINE))

        elif game_data.stats.monsters_killed_level == 1 and \
