...
class Tutorial:
    ...
    def get_messages(game_data, gfx_data):
        ...
        if not game_data.stats.moves:
            ...
            messages.append(
                Message(
                    "Press Tab for help", Pos(x, y + linediff * 2), MessageType.LINE
                )
            )
            messages.append(
                Message(
                    "Press Escape to quit", Pos(x, y + linediff * 3), MessageType.LINE
                )
            )
            if config.conf.crafting:
                messages.append(
                    Message(
                        "V to open crafting", Pos(x, y + linediff * 4), MessageType.LINE
                    )
                )
            if config.conf.consumables:
                messages.append(
                    Message(
                        "I to open invetory", Pos(x, y + linediff * 5), MessageType.LINE
                    )
                )

        elif game_data.player.pos == second_pos:
            messages.append(
                Message("<--- This is your health bar", Pos(0, 20), MessageType.LINE)
            )
            messages.append(
                Message(
                    "<--- This is your shield bar, your shield will reset after this level",
                    Pos(0, 70),
                    MessageType.LINE,
                )
            )
            if not config.conf.keys:
                messages.append(
                    Message(
                        "This is your experience bar", Pos(20, 140), MessageType.LINE
                    )
                )
            else:
                messages.append(
            ...
            if config.conf.consumables:
                ...
                messages.append(
                    Message(
                        "Down to the left are consumables, single use items that can help",
                        Pos(0, y),
                        MessageType.LINE,
                    )
                )
                messages.append(
                    Message(
                        "You can mouse over for more info",
                        Pos(20, y + linediff),
                        MessageType.LINE,
                    )
                )
            ...
            messages.append(
                Message(
                    "You can also melee attack enemies by moving into them, this won't deal much damage though",
                    Pos(x, 360),
                    MessageType.LINE,
                )
            )
            ...
            messages.append(
                Message(
                    "Kill it the second Formula, FRR",
                    Pos(x, y + linediff),
                    MessageType.LINE,
                )
            )
            messages.append(
                Message(
                    "Press 2, or click the formula, to target",
                    Pos(x, y + linediff * 2),
                    MessageType.LINE,
                )
            )
            ...
            messages.append(
                Message(
                    "This is a key, you need these to unlock stairs",
                    Pos(x, y),
                    MessageType.LINE,
                )
            )
            messages.append(
                Message(
                    "Move onto it and press space to pick it up",
                    Pos(x, y + linediff),
                    MessageType.LINE,
                )
            )
            messages.append(
                Message(
                    "When you do, you'll get a little loot",
                    Pos(x, y + linediff * 2),
                    MessageType.LINE,
                )
            )
            ...
            for e in game_data.map.entities:
                if e.stairs and tcod.map_is_in_fov(game_data.fov_map, e.pos.x, e.pos.y):
                    ...
                    messages.append(
                        Message("When you ascend:", Pos(sx, sy + 50), MessageType.LINE)
                    )
                    sy += 75
                    messages.append(
                        Message(
                            "1) You will get to change your formulas",
                            Pos(sx + 30, sy),
                            MessageType.LINE,
                        )
                    )
                    messages.append(
                        Message(
                            "2) Your cooldowns will reset",
                            Pos(sx + 30, sy + linediff),
                            MessageType.LINE,
                        )
                    )
                    messages.append(
                        Message(
                            "3) Your shield will remain",
                            Pos(sx + 30, sy + linediff * 2),
                            MessageType.LINE,
                        )
                    )
                    break
            ...
            has_not_looted = (
                game_data.stats.num_looted_monsters == 0 and not config.conf.keys
            )
            ...
            if has_killed and (has_not_looted or has_cooldown):
                ...
                messages.append(
                    Message(
                        "Now your formula is on cooldown",
                        Pos(20, 220),
                        MessageType.LINE,
                    )
                )
