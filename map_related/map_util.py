...
def get_monster(x, y, game_map, room, monster_choice, assets, entities):
    def create_pack(hp, defense, power, xp, asset, name):
        ...
        for w in range(packsize):
            ...
            fighter_component = Fighter(
                hp=hp, defense=defense, power=power, xp=xp // packsize
            )
            ai = MeleeMonsterAI()
    ...
    # easy
        ...
        monsters.extend(
            create_pack(hp=5, defense=0, power=1, xp=40, asset=assets.dog, name="Hound")
        )
    ...
    # medium
        ...
        monsters.extend(
            create_pack(
                hp=15, defense=2, power=3, xp=60, asset=assets.boar, name="Boar"
            )
        )
